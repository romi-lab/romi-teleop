...
class RobotiqGripper:
    ...
    def move(self, position: int, speed: int, force: int) -> Tuple[bool, int]:
        ...
        # Some URCap socket servers acknowledge multi-variable SET commands without applying every field.
        # Send each variable separately so PRE/GTO reliably update on the controller.
        set_ok = (
            self._set_var(self.POS, clip_pos)
            and self._set_var(self.SPE, clip_spe)
            and self._set_var(self.FOR, clip_for)
            and self._set_var(self.GTO, 1)
        )
        return set_ok, clip_pos
