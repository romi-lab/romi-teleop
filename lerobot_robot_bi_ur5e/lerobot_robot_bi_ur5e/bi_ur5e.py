"""Bimanual UR5e robot wrapper built from two existing UR5e robot instances."""
...
from lerobot_robot_ur5e import UR5E

from .config_bi_ur5e import BiUR5EConfig

logger = logging.getLogger(__name__)


class BiUR5E(Robot):
    """Two UR5e arms exposed as one LeRobot robot with left_/right_ feature prefixes."""

    config_class = BiUR5EConfig
    name = "bi_ur5e"

    def __init__(self, config: BiUR5EConfig):
        ...
        self.left_arm = UR5E(left_arm_config)
        self.right_arm = UR5E(right_arm_config)
        self._last_action_debug_log_s = 0.0

        # Compatibility for code that expects a robot.cameras mapping. The child
        # UR5E instances still own connection and reads; prefixed names avoid
        # collisions between left and right cameras.
        self.cameras = {
    ...
    def send_action(self, action: dict[str, float]) -> dict[str, float]:
        ...
        if now_s - self._last_action_debug_log_s > 1.0:
            self._last_action_debug_log_s = now_s
            logger.info(
                "BiUR5E send_action gripper targets: left=%s right=%s",
                left_action.get("gripper"),
                right_action.get("gripper"),
            )
